...
## Test locations
- `api/Test/test_app.py` — Flask app init and health endpoint
- `api/Test/test_pdf_analysis.py` — End-to-end PDF upload integration test (uses real PDFs from `SimulCuve/data/referenceOrders/`)
- `api/Test/test_excel_generator.py` — Excel cost calculation validation
- `api/Test/test_print_redirect.py` — Print capture / audit log system
- `SimulCuve/Test/test_scale_calibrator.py` — Stage 5 scale calibration
- `SimulCuve/Test/test_detection_images.py` — YOLO detection output (300 DPI, separate openings vs corners)
- `SimulCuve/Test/test_geometry_detector.py` — Geometric shape detection
- `SimulCuve/Test/test_manual_selection.py` — User-drawn region selection
- `SimulCuve/Test/test_composition_block_detection.py` — 16-test suite for composition block detection pipeline (Stages A–D); patches `easyocr.Reader` to avoid model loading
