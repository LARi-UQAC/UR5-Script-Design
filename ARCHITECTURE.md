...
| `trajectory.py` | Cycle generators (`circular_cycle`, `linear_cycle`, `triangular_cycle`) in the plate frame (mm); also read through `get_settings()` at call time. Hosts `resample_points()`, which used to live in `app.py`: regenerating a trial headlessly needs it, and importing the UI to get it would pull matplotlib and its backend. |
| `trial_config.py` | Replays the trial of record described by the versioned `etalement_trial.json` without any UI: `load_trial_config()`, `trial_settings()` (the trial's own overrides, deliberately independent of the workstation's `etalement_settings.json`), and `build_trial_cycles()`. Reached from the CLI as `--export-trial`. This is what makes the committed `etalement.script` regenerable byte for byte rather than an artifact nobody can reproduce (F10). |
| `export.py` | Emits `etalement.script` and `etalement.urp`, reading `Settings` at call time in `_build_urscript_lines()` instead of importing constants by value; prepends a `# === RECETTE DE REPRODUCTION ===` block to EVERY export (`_recipe_header_lines`), naming the cycle count, each cycle's type, waypoint count and label, then the deviations from defaults (`_settings_header_lines`, the only conditional part) and the settings fingerprint. The block carries no date: a timestamp is what made an export non-reproducible, which had forced the block to be emitted only on deviation, leaving the commonest export recording nothing (F10). Owns the CB3 constraints, the PolyScope memory budget check, and the TCP speed clamp. `generate_urscript()` / `generate_urp()` take a `settings=` override and a `force=` flag: `check_overwrite()` compares the output file's digest against `.etalement_export_state.json` (the record of the last export) and refuses to overwrite a hand-edited file unless `force=True`. The program never actuates the 2F-85 (passive finger support: no `rq_*`, no `set_payload`, no tool RS485). |
...
| `app.py` | matplotlib UI, widget wiring, `main()` entry point (`--export`, `--export-urp`, `--export-trial`, `--no-show`, `--force`). The discretization and cycle-shape sliders are views onto `Settings` rather than separate state; the Paramètres button opens `ui_settings.open_settings_window()`. |
...
### `datalogger/` and `onrobot/` — trial data acquisition, two independent paths
...
`onrobot/acq_logger_daemon.py` imports the standard library and nothing else - not
...
| `test_export_reproducibility.py` | The four checks F10 asks for: the headless export matches the golden fixture, regenerating the trial from `etalement_trial.json` reproduces `etalement.script` byte for byte, the recipe header's announced waypoint counts match the `movel` actually emitted per cycle, and comparing an export against the wrong one of the two references fails with a message naming which is which |
...
Two reference artifacts coexist, and confusing them is exactly what F10 blamed the repository
for. `tests/fixtures/golden_headless.script` is the headless `_build_urscript_lines()`
output at defaults, subsampled to `URSCRIPT_N_WAYPOINTS_CIRCULAR` and rectilinear on cycles 4
to 6; it is the byte-for-byte identity witness. The committed `etalement.script` is the trial
of record, produced through the interface export path with 200 waypoints per circular cycle
and triangular cycles 4 to 6, and it is regenerated from `etalement_trial.json`. Both were
regenerated on 2026-08-16 when the recipe header landed, in LF, in one deliberate commit
alongside the `CIRC_R_CIRCLE` and `CIRC_N_CIRCLES` default alignment.
