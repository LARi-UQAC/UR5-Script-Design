"""
RTDE emulator - wire layer: package framing, the output recipe, the runtime-
state machine, and trajectory sampling.

Split out of ur5_sim/rtde_server.py to stay under the project's per-file
token ceiling (.claude/rules/code-style.md); the split has no behaviour of
its own, it only moves pure helpers and constants that carry no socket
state. ur5_sim/rtde_server.py holds RtdeServer, the class that owns the
listening socket and the emitter thread, and imports every name below by
its explicit name.
