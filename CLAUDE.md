...
**C-language exception.** `datalogger/` holds the RTDE fallback monitor, a standalone tool for a lab computer that has no Python and where nothing can be installed (isolated VLAN, no internet), so both the tool and its tests are C. `datalogger/tests/test_rtde_fallback_monitor.c` includes the tool's single translation unit with its `main()` compiled out, so it calls the real functions instead of shelling out. It is not collected by `unittest discover`; run `datalogger\tests\build_and_run_tests.bat` (needs MinGW-w64 `gcc` on `PATH`). It covers big-endian decoding against known byte sequences, every ordered `runtime_state` transition pair, the 20 ms decimation grid, the CSV schema, and an integration layer replaying the RTDE handshake from a fake server on loopback. That C tool remains fully standalone: it reads and writes nothing belonging to `etalement*.script`, `etalement*.urp`, `ur5_sim` or the design UI.

**`datalogger/` now holds two languages.** Beside the C fallback monitor sit the Python files of the on-robot main path: `acq_logger_daemon.py` (the daemon that receives 50 Hz samples from `etalement_acq.script` over loopback port 50100 and writes `ACQ_log_*.csv` to the USB key), `urmagic_acqlogger.sh` (the UR magic file that launches it as root on key insertion), and `acq_emulator.py` (development machine only: fake FT-300 plus a fake robot client replaying `etalement.script`). The daemon is tested by `tests/test_acq_logger_daemon.py`, which IS collected by `unittest discover`; only the C harness stays outside it. The daemon is Python 2.7 compatible, imports the standard library and nothing else, and is loaded in tests by path rather than by import, because it is copied alone onto a USB key. See ARCHITECTURE.md, sections 2 and 8.
